...
public class ControlCore {
	...
	/**
	 * 登陆验证 return 0 登陆失败 return 1 登陆成功
	 */
	public int loginJudge(String userName, String password) {
		...
		String otherUnameSha1 = aesUtil.decryptString(
				aesUtil.parseHexStr2Byte(mainUser.getUserNameFinalString()),
				password);
		String otherPdSha1 = aesUtil
				.decryptString(aesUtil.parseHexStr2Byte(mainUser
	...
	public void saveXmlData() throws IOException {
		...
		pmUserElement.addAttribute("userName",
				encryptMsg(mainUser.getUserName(), mainUser.getUserPassword()));
		// 此处应该将用户密码hash存储
		pmUserElement.addAttribute(
				"password",
				encryptMsg(mainUser.getUserPassword(),
		...
		XmlUtil.doc2XmlFile(document, directory.getAbsolutePath()
				+ "\\src\\sfm\\xml\\msgSave.xml");
	}

	// 读取xml文件的数据到内存和jtable当中
	public void readXmlFileData() {
		Document doc = XmlUtil.load(getClass()
				.getResource("../xml/msgSave.xml").toString());
		try {
	...
	public void readXmlUserData() {
		File directory = new File("");
		File judgeXMLExist = new File(directory.getAbsolutePath()+"\\src\\sfm\\xml\\msgSave.xml");
		...
		Document doc = XmlUtil.load(getClass().getResource("../xml/msgSave.xml").toString());
		try {
